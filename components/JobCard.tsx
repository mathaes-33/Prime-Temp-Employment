...
const JobCard: React.FC<{ job: Job }> = ({ job }) => (
  <Card className="flex flex-col h-full relative">
    ...
    <div className="flex flex-col h-full">
      <p className="text-sm font-medium text-secondary">{sanitizeText(job.company)}</p>
      <h3 className="text-xl font-bold text-dark mt-1">{sanitizeText(job.title)}</h3>
      
      <div className="mt-3 space-y-2 text-sm text-slate-600">
        <div className="flex items-center gap-2">
          <MapPinIcon className="h-4 w-4 flex-shrink-0 text-slate-400" />
          <span>{sanitizeText(job.location)}</span>
        </div>
        <div className="flex items-center gap-2">
          <ClockIcon className="h-4 w-4 flex-shrink-0 text-slate-400" />
          <span>{sanitizeText(job.employmentType)}</span>
        </div>
        {job.salary?.visible && (
             <div className="flex items-center gap-2">
                <DollarSignIcon className="h-4 w-4 flex-shrink-0 text-slate-400" />
                <span>{job.salary.min.toLocaleString()} - {job.salary.max.toLocaleString()} {sanitizeText(job.salary.currency)}</span>
            </div>
        )}
      </div>

      <p className="mt-4 text-slate-500 text-sm flex-grow">{sanitizeText(job.description).substring(0, 90)}...</p>
